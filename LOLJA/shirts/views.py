...
def allprod(request):
    ...
    context = {'prod': products, 'products': products, 'occ': occ, 'sleeve': sleeve,
               'neck': neck, 'ideal': ideal, 'brand': brands, 'colors': colors, 'page': page}
...
def product_detail(request, slug):
    ...
    # This is discount equation
    sell_price = floor(size_price - (size_price * (details.discount)/100))
    context = {'details': details, 'price': size_price,
...
def update_cart_for_anom_user(request, id, size, method):
    # Here method is for checking either it is incrementing cart or decrementing cart by taking arguments as plus or minus
    user = None
    if request.user.is_authenticated:
        user = request.user
    cart = request.session.get('cart')
...
def remove_cart(request, id, size):
    user = None
    if request.user.is_authenticated:
        user = request.user
    cart = request.session.get('cart')
...
def add_cart_to_database(user, size, tshirt):
    ...
    existing = Cart.objects.filter(user=user, sizevariant=size)
    if len(existing) > 0:
        # This means zero index mtlb jo sbse pehle hoga whi waala
        obj = existing[0]
        # if quantity of any product is already exist with the same size so its quantity will increase in the cart.
        obj.quantity = obj.quantity + 1
        obj.save()
...
def orders(request):
    ...
    orders = order.objects.filter(user=user).order_by(
        '-date').exclude(order_status='PENDING')
...
def admin_dashboard(request):
    ...
    context = {'tshirtcount': tshirtcount,
               'order': ordercount, 'user': usercount}
